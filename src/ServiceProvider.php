...
class ServiceProvider implements ServiceProviderInterface
{
    ...
    public function register(Container $pimple)
    {
        ...
        // 旺店通旗舰版 OpenApi 接口（使用新的签名算法，与企业版不同）
        // 企业版使用：$wdt->trade, $wdt->basic 等
        // 旗舰版使用：$wdt->qijianApi
        $pimple['qijianApi'] = function ($pimple) {
            $config = $pimple->getConfig();

            return new OpenApi($config['appkey'], $config['appsecret'], $config['sid'], $config['baseUrl']);
        };
    }
}
